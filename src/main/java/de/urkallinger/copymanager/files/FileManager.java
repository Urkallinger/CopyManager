...
public class FileManager {
    ...
    public void readFiles(File rootDir) throws FileReaderInProgressException {
    ...
    public void readFiles(File rootDir, ParamCallback<List<FileListItem>> callback)
        ...
        if (!fileReader.isAlive()) {
            Runnable reader = new FileReader(rootDir, files -> {
                setFileCache(files);
                callback.call(files);
            });
    ...
    public void copyFiles(final List<FileListItem> files, final File targetDir) throws FileCopierInProgressException {
    ...
    public List<FileListItem> getFiteredFileCache() {
        return fileCache.stream().filter(item -> extensions.contains(item.getExtension())).collect(toList());
    }
